...
export class WebDFU {
	...
	async isError() {
		try {
			...
			return state?.state == DFUDeviceState.dfuERROR;
		} catch (_) {
			return true;
		}
	}
	...
	private async do_dfuse_write(process: WebDFUProcessWrite, xfer_size: number, data: ArrayBuffer) {
		...
		await this.poll_until(state => state == DFUDeviceState.dfuMANIFEST);
	}
	...
	private async dfuseCommand(command: number, param = 0x00, len = 1) {
		...
		try {
			...
			throw new WebDFUError(
				"Error during special DfuSe command " + commandNames[command] + ":" + error
			);
		}
